...
printf "%s\n" "${YELLOW}Starting yarn audit fix${NORMAL}";
...
for package in $(find . -maxdepth 3 -type f -name "package.json"); do
    ...
    printf "%s\n" "${BRIGHT}    Navigating to $rp to run yarn audit fix${NORMAL}";    
    cd $rp;
    printf "%s\n" "${BRIGHT}    Running yarn audit fix${NORMAL}";
    #Make sure yarn audit-fix exists
    yarn remove yarn-audit-fix
