...
printf "%s\n" "${YELLOW}Starting npm-check${NORMAL}";
...
for package in $(find . -maxdepth 3 -type f -name "package.json"); do
    ...
    printf "%s\n" "${BRIGHT}    Navigating to $rp to run npm-check${NORMAL}";    
    cd $rp;
    printf "%s\n" "${BRIGHT}    Running npm-check${NORMAL}";
    logfile=$tmpfolder/yarn-$mainfolder-unused-dependencies.log
